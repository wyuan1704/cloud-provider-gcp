...
func NewEventHandler(podInformer coreinformers.PodInformer, nodeInformer coreinformers.NodeInformer, verifier Verifier, hmsSyncNodeURL string, client clientset.Interface) (*nodeHandler, error) {
...
func (nh *nodeHandler) process(ctx context.Context, key string) error {
	...
	if !exist {
		ctxlog.Infof(ctx, "Node %q doesn't exist", key)
	...
	if !ok {
		return fmt.Errorf("invalid pod object from key %q: %#v", key, oNode)
	}
...
func (ng *nodeGSAs) sync(ctx context.Context, client *hms.Client, zone string, curGSAs []string) error {
	...
	if equal := reflect.DeepEqual(ng.lastGSAs, curGSAs); equal {
